...
def test_render_weight_4d_default_lays_kernels_across_in_channels() -> None:
    ...
    assert strip is not None
    # 3 kernel tiles of 3×3, captioned by the tiled `in` axis (dim 1).
    assert _tile_sizes(strip) == [(3, 3)] * 3
    assert (strip.tiles[0].width, strip.tiles[0].height) == (TILE_SIZE, TILE_SIZE)
    assert [t.label for t in strip.tiles] == ["DIM 1: 0", "DIM 1: 1", "DIM 1: 2"]
...
def test_render_weight_custom_tile_axis_changes_tile_count() -> None:
    ...
    # The caption names the chosen tile axis (dim 0) rather than channels.
    assert strip.tiles[0].label == "DIM 0: 0"
...
def test_patch_grid_is_one_column_per_channel() -> None:
    tp = _type_patches(torch.zeros(2, 5), torch.rand(2, 5, 3, 4, 4))
    grid = render_patch_grid(tp)
    assert grid is not None
    # One captioned column per channel; each stacks its 5 sample cells of 4×4
    # with 1-px gaps, shown PATCH_CELL_SIZE wide.
    assert len(grid.columns) == 2
    assert _decode(grid.columns[0].image).size == (4, 5 * 4 + 4)
    assert (grid.columns[0].width, grid.columns[0].height) == (
        PATCH_CELL_SIZE,
...
def test_patch_grid_is_png_regardless_of_strip_format() -> None:
    ...
    grid = render_patch_grid(tp)
    assert grid is not None
    assert all(c.image.startswith(b"\x89PNG") for c in grid.columns)
...
def test_patch_grid_denormalizes_with_mean_std() -> None:
    ...
    assert grid is not None
    assert _rgb_at(_decode(grid.columns[0].image), 0, 0) == (127, 127, 127)
...
def test_patch_grid_marks_unfilled_slots_gray() -> None:
    ...
    grid = render_patch_grid(tp)
    assert grid is not None
    img = _decode(grid.columns[0].image)
...
def test_patch_grid_heatmap_tints_red_or_blue(sign: float, channel: int) -> None:
    ...
    assert grid is not None
    rgb = _rgb_at(_decode(grid.columns[0].image), 0, 0)
...
def test_patch_grid_heat_legend_only_when_heatmap_enabled() -> None:
    ...
    # Display-resolution colorbar matching a column's CSS height.
    assert _decode(grid.heat_legend).size[1] == grid.columns[0].height
