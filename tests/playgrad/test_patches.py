...
def test_crop_side_clamps_to_min_and_image() -> None:
    assert crop_side(4, 32) == 24  # ceil(8) * 3
    assert crop_side(32, 32) == 8  # 3 -> floored at MIN_PATCH
    assert crop_side(2, 32) == 32  # 48 -> capped at the image side
...
def test_max_pixel_crop_geometry() -> None:
    ...
    # act 4x4 -> input 32x32: center (20, 28), 24x24 window clamped to
    # top=8, left=8 (left would be 16 but 32-24=8 caps it).
    assert tp.top[0, 0].item() == 8
    assert tp.left[0, 0].item() == 8
    assert torch.equal(tp.patches[0, 0], x[1, :, 8:32, 8:32])
...
@pytest.mark.parametrize("corner", [(0, 0), (3, 3)])
def test_border_extremes_stay_in_bounds(corner: tuple[int, int]) -> None:
    ...
    assert 0 <= top <= 32 - 24
    assert 0 <= left <= 32 - 24
    assert tp.patches.shape[-2:] == (24, 24)
...
def test_buffer_shapes() -> None:
    ...
    assert pixel.patches.shape == (3, N_PER_CHANNEL, 3, 12, 12)  # ceil(4)*3
